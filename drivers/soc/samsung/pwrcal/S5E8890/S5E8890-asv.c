...
static int dvfsbig_get_asv_table(unsigned int *table)
{
	...
	for (lv = 0; lv < max_lv; lv++) {
		...
		if (lv > 18) {
                	if (table[lv-1] - (asv_vol_step * 4) > 575000)
	...
	for (lv = 2; lv >= 0; lv--)
		if (table[lv+1] + (asv_vol_step * 4) < 1450000)
			table[lv] = table[lv+1] + (asv_vol_step * 4);
		else
			table[lv] = 1450000;
...
static int dvfslittle_get_asv_table(unsigned int *table)
{
	...
	for (lv = 0; lv < max_lv; lv++) {
		...
		if (lv > 14) {
                	if (table[lv-1] - (asv_vol_step * 4) > 550000)
				table[lv] = table[lv-1] - (asv_vol_step * 4);
			else
				table[lv] = 550000;
		}
	...
	for (lv = 3; lv >= 0; lv--)
		if (table[lv+1] + (asv_vol_step * 4) < 1300000)
			table[lv] = table[lv+1] + (asv_vol_step * 4);
		else
			table[lv] = 1300000;
